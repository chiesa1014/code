...
from keras.layers import MultiHeadAttention, Add, Flatten
...
# 保存种子到文件
seed_file_path = 'transformer_seed（323033）.txt'
...
# # 读取种子文件
# with open('transformer_seed（72728）.txt', 'r') as f:
...
#输入的历史look_back步 ，和预测未来的T步，这里的looc_back想用历史几天的数据就输入几天，T不用改
look_back = 5
T = 1
...
class TransformerTimeSeriesModel(models.Model):
...
input_shape = (look_back, 5)  # Assuming 5 features as per the loaded data
inputs = Input(shape=input_shape)
transformer_output = TransformerTimeSeriesModel(num_blocks=3, embed_dim=32, dense_dim=64, num_heads=4, dropout_rate=0.1, output_sequence_length=T)(inputs)
...
test_cycle_adjusted = cycle_test['cycle'].values[look_back + T - 1:end_index:T]
...
plt.plot(test_cycle_adjusted, test_predict.flatten(), label='Predicted')
...
plt.plot(val_cycle_adjusted, val_predict.flatten(), label='Predicted')
plt.xlabel('Cycle', fontsize=14)
plt.ylabel('Discharge Capacity (QD)', fontsize=14)
plt.legend(fontsize=12, loc='upper right')
plt.show()
...
#
# # 保存DataFrame到CSV文件
# train_result_df.to_csv('Transformer-traindata(101516).csv', index=False)
#
# 创建DataFrame
...
# 保存DataFrame到CSV文件
test_df.to_csv('Transformer-testdata(323033).csv', index=False)
...
# 保存DataFrame到CSV文件
val_df.to_csv('Transformer-valdata(323033).csv', index=False)
#
#
# # 读取CSV文件
